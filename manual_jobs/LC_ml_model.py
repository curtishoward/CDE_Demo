...
df = spark.sql("SELECT * FROM default.LC_Table_" + suffix)
...
predictions.write.format('parquet').mode("overwrite").saveAsTable('default.LC_model_scoring_' + suffix)

## Storing Model Pipeline to S3
pipelineModel.write().overwrite().save(s3BucketName + "/pipeline_" + suffix)
